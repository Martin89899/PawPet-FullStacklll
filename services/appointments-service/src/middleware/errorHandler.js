...
/**
 * Manejador centralizado de errores para el Appointments Service
 * @class ErrorHandler
...
const errorHandler = async (err, req, res, next) => {
  ...
  // Errores de conflictos (ej: doble reserva)
  ...
  if (err.name === 'AvailabilityError') {
    return res.status(422).json({
      success: false,
      error: 'Availability Error',
      message: err.message || 'Horario no disponible'
    });
  }
...
/**
 * Clase personalizada para errores de conflictos
 * @class ConflictError
 * @extends Error
 */
...
/**
 * Clase personalizada para errores de disponibilidad
 * @class AvailabilityError
 * @extends Error
 */
class AvailabilityError extends Error {
  constructor(message = 'Horario no disponible') {
    super(message);
